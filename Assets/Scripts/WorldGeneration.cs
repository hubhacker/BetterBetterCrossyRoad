    ...
    void Update()
    {
        ...
        if (currentZ > lastGeneratedZ)
        {
            ...
            for (int i = 0; i < 4; i++){
                ...
                Vector3 treeSpawnPosition = new Vector3(Random.Range(-10, 10), 0, currentZ + spawnOffset);
                Instantiate(randomTree, treeSpawnPosition, Quaternion.identity);

                // prevent overlapping trees
                // for (int j = 0; j < 10; j++)
                // {
                //     treeSpawnPosition = new Vector3(Random.Range(-10, 10), 0, currentZ + spawnOffset);
